...
def k_instantiate(config, *args, is_func=False, key = 'k_target', **kwargs):
    assert key in config, f'Config should have {key} for class instantiation.'
    config['_target_'] = config.pop(key)
    target = config['_target_']
