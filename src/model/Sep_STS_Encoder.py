...
# the code seem come from Swim-Transformer: https://github.com/microsoft/Swin-Transformer
class Mlp(nn.Module):
...
class SepSTSBlock(nn.Module):
    ...
    def __init__(
        ...
        dim,
        num_heads,
        depth_window_size=(1, 8, 8),  # (D, H, W)
        shift_size=(0, 0, 0),
        point_window_size=(4, 1, 1),  # (D, H, W)
        mlp_ratio=4.0,
        qkv_bias=True,
        qk_scale=None,
        act_layer=nn.GELU,
...
class BasicStem(nn.Sequential):
    ...
    def __init__(self, dim):
        super().__init__(
            nn.Conv3d(3, dim, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1), bias=False),
...
class SepSTSLayer(nn.Module):
    def __init__(self, plane, depth, num_frames, num_heads, window_size):
        ...
        self.upper = SepSTSBasicLayer(
            plane,
            depth=depth,
            num_heads=num_heads,
            depth_window_size=window_size,
            point_window_size=(num_frames, 1, 1),
        )
...
class ResBlock(nn.Module):
    """
    3D version of Easy ResBlock, keep the channel fixed.
    """
...
class SepSTS_Encoder(nn.Module):
    def __init__(self, nf, NF, window_size, nh):  # NF is the number of input frames!
        super(SepSTS_Encoder, self).__init__()
        self.stem = nn.Sequential(
            nn.Conv3d(in_channels=3 + 2, out_channels=nf[-1] // 2, kernel_size=3, stride=1, padding=1),
    ...
    def forward(self, x):  # B,3,D,H,W
        x0 = self.stem(x)  # B,16,D,H,W

        x1 = self.down0(x0)  # B,32,D,H/2,W/2
        x1 = self.stage1(x1)  #
        ...
        return x0, x1, x2, x3, x4  # return the intermediate of different stage, 5 in total
