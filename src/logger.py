...
class BatchMetrics:
    def __init__(self, *keys, postfix="", writer=None):
        ...
        self.scalar_dic = {}  # it's a recoder to record scalars group
        self.reset()
    ...
    def update_scalars(self, tag, scalar_dict: dict, n=1):
        if self.postfix:
            tag = tag + self.postfix

        if self.writer is not None:
            self.writer.add_scalars(tag, scalar_dict)
        ...
        for key, value in scalar_dict.items():
            ...
            # scalars group set
            if tag in self.scalar_dic:
                self.scalar_dic[tag].add(key)
            else:
                self.scalar_dic[tag] = set()
    ...
    def write_epoch(self):
        data_avg, scalar_avg = dict(self._data.average), {}
        for tag, k_set in self.scalar_dic.items():
            scalar_avg[tag] = {}
            for key in k_set:
                scalar_avg[tag][key] = data_avg.pop(key)

        for k, v in data_avg.items():
            self.writer.add_scalar(k + "/epoch", v)

        for k, v in scalar_avg.items():
            self.writer.add_scalars(k + "/epoch", v)

        return data_avg, scalar_avg
...
class EpochMetrics:
    ...
    def minimizing_metric(self, idx):
        ...
        try:
            ...
            self.logger.warning("Warning: Metric '{}' is not found. " "Model performance monitoring is disabled.".format(self.monitor_metric))
